...
NorrisBot.prototype._performRequest = function (originalMessage){
	...
	var options = {
		host : 'api.chucknorris.io', // the domain name
		path : '/jokes/random', // the rest of the url with parameters if needed
		method : 'GET' // do GET
	};
...
/*
	...
	while (responseObject != null){
	    ...
	    if (channel == null){
	    	channel = self._getGroupById(originalMessage.channel);
	    	self.postMessageToGroup(channel.name, responseObject.value, {as_user: true}); 
	    	console.log(responseObject.value);
	    	break;
	    } else {
	    	 self.postMessageToChannel(channel.name, responseObject.value, {as_user: true});
	    	 console.log(responseObject.value); 
	    	 break;
	    }
